...
namespace LCUSharp
{
    ...
    public class LeagueClientApi : ILeagueClientApi
    {
        ...
        public static async Task<LeagueClientApi> ConnectAsync()
        {
            ...
            var (wssPort, wssToken) = await LockFileHandler.ParseLockFileAsync(_processHandler.ExecutablePath).ConfigureAwait(false);
            var eventHandler = new LeagueEventHandler(wssPort, wssToken);
        ...
        private static async Task<(int port, string token)> GetAuthCredentialsAsync()
        {
            ...
            return await _processHandler.GetDataFromProcess()!.ConfigureAwait(false);
        }
