...
namespace LCUSharp.Utility
{
    ...
    /// </summary>
    public static class LockFileHandler
    {
        ...
        public static async Task<(int port, string token)> ParseLockFileAsync(string path)
        {
            var lockfilePath = await WaitForFileAsync(path).ConfigureAwait(false);
            await using (var fileStream = new FileStream(lockfilePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
            {
